...
- **mode** (`AgentBody.mode`) — *how* it runs: the loop / prompting method, one of
  ...
  `react`). Pick one.
- **tools** (`AgentBody.tools`) — ordinary tools (from `agent_compose.tools`, e.g.
...
from agent_compose.nodes.agent.modes import MODES, AgentRunContext
from agent_compose.nodes.base import Node, NodeKind, NodeResult
...
class AgentNode(Node):
    ...
    def _build_model(self) -> Any:
        ...
        from agent_compose.llm_clients import model_from_config
